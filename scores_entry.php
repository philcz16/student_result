...
include("dbconnect.php");
	...
	if(isset($_POST['search'])){
		...
		$query= mysqli_query($con,"SELECT * FROM `subject_registered` WHERE subname LIKE '%$subname%' AND class_name='$class_name'");
		if (mysqli_num_rows($query) == 0 || !isset($class_name) || !isset($subname)) {
		$_SESSION['errormsg'] = "<div id='errormsg'>Data doesn't exist for this class</div>"; 
		header("Location: search_class.php");
  }
		...
		$term=  mysqli_real_escape_string($con,$_POST['term']);
          ...
          if($check != 0){
          	...
          	if(($first_ca)>10 || ($second_ca)>10 || ($third_ca)>10){
				...
				if(isset($_POST['submit']) && ($session_yr!=="..Select Session..") && ($term!=="..Select Term..")){
...
<html lang="en-US">
   ...
   <div id="wrapper">
      <section>
		...
		<?php $query1 = mysqli_query($con, "SELECT * FROM `sub_senior` where subname like '$subname'");
		...
		<input type="search" name="subname" value="<?php echo $sub;?>" readonly class="subj">
		...
		<?php
		$count = 0;
		$sql = mysqli_query($con, "SELECT * FROM `subject_registered` WHERE subname LIKE '%$subname%' AND class_name='$class_name'");
